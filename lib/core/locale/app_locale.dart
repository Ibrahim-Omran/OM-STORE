...
class AppLocalizations {
  ...
  static const LocalizationsDelegate<AppLocalizations> delegate = _AppLocalDelegate();
  static const String _path = 'assets/translations/';

  late Map<String, String> _localStrings;

  Future<void> loadJsonFiles() async {
    String encodedString = await rootBundle.loadString("$_path${locale!.languageCode}.json");
    Map<String, dynamic> jsonMap = jsonDecode(encodedString);
    _localStrings = jsonMap.map((key, value) => MapEntry(key, value.toString()));
  }

  String translate(String key) {
    return _localStrings[key] ?? key; // رجع key لو مفيش ترجمة
  }
}
